...
metadata {
	...
	tiles {
        ...
        standardTile("Zone 7", "device.zone7", inactiveLabel: false) {			
			state "closed", label: 'Zone 7', icon: "st.contact.contact.closed", backgroundColor: "#79b821"
            state "open", label: 'Zone 7', icon: "st.contact.contact.open", backgroundColor: "#ffa81e"
		}  
		...
		standardTile("keyPound", "device.keyPound", width: 1, height: 1, inactiveLabel: false, decoration: "flat") {
			state "default", action:"keyPound", label: '#'
		}         
		}
		standardTile("panic", "device.panic", width: 1, height: 1, inactiveLabel: false, decoration: "flat") {
			state "default", action:"panic", label: 'PANIC'
		}  

    
    main "paneldisplay"
    details(["paneldisplay", "displayupdate", "ArmTile", "silent", "Zone 1", "Zone 2", "Zone 3", "Zone 4", "Zone 5", "Zone 7",
    "refresh", "panic"])
}
...
def armaway() {
    ...
    if( state.bLoud == "False" )
    {
    	//if( state.bBypass == "True" )
    	//{
        //	sendKeypressArray( [ (byte)0x05, (byte)0x0B, (byte)0x27 ] as byte[] )
        //}
        //else
        //{
        	sendKeypressArray( [ (byte)0x05, (byte)0x27 ] as byte[] )
        //}
    }
    else 
    {
    	//if( state.bBypass == "True" )
        //{
        //	sendKeypressArray( [ (byte)0x0B, (byte)0x27 ] as byte[] )
        //}
        //	else
    	//{
    		sendKeypress( 0x27 )
...
def panic()
{
	...
	sendKeypressArray ( [ (byte)0x0C, (byte)0x06 ] as byte[] );
}    
...
def DisplayNames( Integer Index )
{
    ...
    case 0xF9:
    	return "\r\n"
    case 0x7E:
    	return createEvent(name: "Contact Sensor", value: "Open", displayed: true, isStateChange: true)      
    }
    return '@'
}
